...
// Model cho Biến thể món ăn (Size, Lượng đường/đá, v.v.)
...
class ToppingItem {
  ...
  ToppingItem({required this.id, required this.name, required this.price, this.quantity = 0});
...
class MenuItem {
  ...
  double basePrice;
  String category;
  String unit; // ly, đĩa, kg, gram, bịch...
  String? imagePath;
  bool isAvailable; // Trạng thái: Tạm ngừng bán hay Đang bán
...
class _MainHomeScreenState extends State<MainHomeScreen> {
  int _currentIndex = 0;
  String currentRole = 'Admin'; // 'Admin' hoặc 'Nhân viên'
  ...
  // Tự động Reset theo ngày và tháng
  ...
  // Pick ảnh từ thư viện
  Future<String?> _pickImage() async {
    final XFile? image = await _picker.pickImage(source: ImageSource.gallery);
    return image?.path;
  }

  // TÍNH NĂNG 1: THÊM / SỬA / XÓA MÓN & BIẾN THỂ & DẠNG TẠM NGỪNG BÁN
            ...
            child: Column(
              ...
              children: [
                ...
                Row(
                  ...
                  children: [
                    const Text('Biến thể món (Size/Loại)', style: TextStyle(fontWeight: FontWeight.bold)),
                    IconButton(
  ...
  // TÍNH NĂNG 2: CHỌN MÓN, TĂNG GIẢM CÂN NẶNG HẶC SỐ LƯỢNG & TOPPING
          ...
          return AlertDialog(
              ...
              child: Column(
                ...
                children: [
                  if (item.variants.isNotEmpty) ...[
                    const Align(alignment: Alignment.centerLeft, child: Text('Chọn Biến thể/Size:', style: TextStyle(fontWeight: FontWeight.bold))),
            ...
            actions: [
              ...
              ElevatedButton(
                ...
                child: Text('Thêm vào đơn • ${totalItemPrice.toStringAsFixed(0)}đ'),
              )
            ],
          );
        },
  ...
  // TÍNH NĂNG 3: GỘP & TÁCH HÓA ĐƠN VÀ CHIA TIỀN
        ...
        builder: (context, setDlgState) => AlertDialog(
          ...
          content: Column(
            ...
            children: [
              ...
              const Divider(),
              Text('Mỗi người trả: ${(total / people).toStringAsFixed(0)} VNĐ', style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.teal, fontSize: 16)),
            ],
          ),
  ...
  // TÍNH NĂNG 4: THANH TOÁN & GHI NHẬN CHI PHÍ
  void _checkoutTable() {
  ...
  void _addExpenseDialog() {
    ...
    showDialog(
      context: context,
      builder: (ctx) => AlertDialog(
        title: const Text('Ghi Nhận Chi Phí (Lợi Nhuận Ròng)'),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            TextField(controller: noteCtrl, decoration: const InputDecoration(labelText: 'Nội dung chi tiêu')),
            TextField(controller: amountCtrl, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Số tiền chi (VNĐ)')),
          ],
        ),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('Hủy')),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Quản Lý Bán Hàng ($currentRole)'),
        actions: [
          IconButton(
  ...
  Widget _buildSalesTab() {
    ...
    return Column(
      children: [
        ...
        // Bảng đơn hàng của bàn hiện tại
        Card(
      ...
      child: Scaffold(
        ...
        body: TabBarView(
          children: [
            ...
            ListView(
              children: [
                const ListTile(title: Text('Nhật Ký Doanh Thu Theo Ngày (Đã Chốt)', style: TextStyle(fontWeight: FontWeight.bold))),
                ...dailyRevenueLogs.map((e) => ListTile(title: Text(e['date']), trailing: Text('${e['revenue']}đ'))),
                const Divider(),
                const ListTile(title: Text('Nhật Ký Doanh Thu Theo Tháng (Đã Chốt)', style: TextStyle(fontWeight: FontWeight.bold))),
      ...
      child: Column(
        ...
        children: [
          ...
          ElevatedButton(
            ...
            child: Text('Đổi sang quyền ${currentRole == 'Admin' ? "Nhân viên" : "Admin"}'),
          )
        ],
      ),
    );
